...
breaking change in `@cognite/auth-wrapper` it will get a major version bump, if you use the `BREAKING
CHANGE: desc` footer. However, packages that depends on `@cognite/auth-wrapper` will only get a patch
...
When CI/CD sees this, it will use semantic-release to update
package version based on commit messages, and add the
...
Let's say you want to make a fix to `@cognite/auth-wrapper@0.1.0`,
after `@cognite/auth-wrapper@0.3.0` is already published.

First check if there already is a backporting branch called `@cognite/auth-wrapper@0.2.x`.
```bash
git fetch
git checkout @cognite/auth-wrapper@0.2.x
```

If there isn't, make it based on the latest release of MAJOR version 0.2.
You can look at git tags to find the last release.
```bash
git fetch --all --tags
git tag | grep @cognite/auth-wrapper@0.2.x.
```

Let's say `0.2.6` was the last release before `0.3.0`.
We make a branch based on it for all backporting needs.

```bash
git checkout tags/@cognite/auth-wrapper@0.2.6 -b @cognite/auth-wrapper@0.2.x
```
